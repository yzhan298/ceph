...
int KernelDevice::flush()
{
  ...
  dout(1) << __func__ << " ### fd=" << fd_directs[WRITE_LIFE_NOT_SET] << dendl;
  int r = ::fdatasync(fd_directs[WRITE_LIFE_NOT_SET]); // system call to flush only opened data(not include metadata)
...
void KernelDevice::_aio_thread()
{
  ...
  while (!aio_stop)
  {
    ...
    if (r > 0)
    {
      ...
      for (int i = 0; i < r; ++i)
      {
        ...
        if (aio[i]->queue_item.is_linked())
        {
          ...
          debug_aio_unlink(*aio[i]); // remove the object(because it's finished)
        }
        ...
        dout(30) << __func__ << " finished aio " << aio[i] << " r " << r
                 << " ioc " << ioc
    ...
    dout(1) << __func__ << " ### _aio_thread finish time=" << ceph_clock_now() << dendl;
  }
  reap_ioc();
