...
OSDService::OSDService(OSD *osd) : osd(osd),
                                   cct(osd->cct),
...
OSD::OSD(CephContext *cct_, ObjectStore *store_,
         ...
         MonClient *mc,
         const std::string &dev, const std::string &jdev) : Dispatcher(cct_),
...
int OSD::init()
{
  ...
  dout(0) << "using " << op_queue << " op queue with priority op cut off at " << op_prio_cutoff << "." << dendl;
...
int OSD::mon_cmd_maybe_osd_create(string &cmd)
{
  ...
  while (true)
  {
    ...
    if (r < 0)
    {
      if (r == -ENOENT && !created)
      {
        string newcmd = "{\"prefix\": \"osd create\", \"id\": " + stringify(whoami) + ", \"uuid\": \"" + stringify(superblock.osd_fsid) + "\"}";
    ...
    spg_t pgid)
{
  ...
  else
  {
    ...
    decode(name, p);
    if (p.end())
    { // dev release v13.0.2 did not include ec_profile
    ...
    PastIntervals *pi)
{
  ...
  *h = pg_history_t(created, created_stamp);
  ...
  dout(10) << __func__ << " " << *h << " " << *pi
           << " [" << (pi->empty() ? pair<epoch_t, epoch_t>(0, 0) : pi->get_bounds()) << ")"
           << dendl;
}
...
void OSD::ms_fast_dispatch(Message *m)
{
  ...
  {
    m->put(); // reduce ref. if 0, release space. 
    return;
  }
  ...
  //convert message to OpRequest
  //op type typedef boost::intrusive_ptr<OpRequest> Ref
      ...
      deadline(timestamp)
{
  ...
  if (!must)
  {
    double scrub_min_interval = pool_scrub_min_interval > 0 ? pool_scrub_min_interval : cct->_conf->osd_scrub_min_interval;
    double scrub_max_interval = pool_scrub_max_interval > 0 ? pool_scrub_max_interval : cct->_conf->osd_scrub_max_interval;
  ...
  if (service.first_scrub_stamp(&scrub))
  {
    do
    {
      ...
      dout(10) << "sched_scrub scrubbing " << scrub.pgid << " at " << scrub.sched_time
               << (pg->get_must_scrub() ? ", explicitly requested" : (load_is_low ? ", load_is_low" : " deadline < now"))
               << dendl;
      if (pg->sched_scrub())
      {
        pg->unlock();
                                     ...
                                     bool is_fast_dispatch)
{
      ...
      (map->get_cluster_addrs(from) != m->get_source_addrs()))
  {
    dout(5) << "from dead osd." << from << ", marking down, "
            ...
            << " expected "
            << (map->is_up(from) ? map->get_cluster_addrs(from) : entity_addrvec_t())
            << dendl;
    ConnectionRef con = m->get_connection();
    con->mark_down();
    if (auto s = ceph::ref_cast<Session>(con->get_priv()); s)
...
{
  const utime_t stamp = op->get_req()->get_recv_stamp(); // message receive time
  const utime_t latency = ceph_clock_now() - stamp; // from receive to enqueue
  ...
  logger->tinc(l_osd_op_before_queue_op_lat, latency); // very small
  op->set_enqueued_time(ceph_clock_now()); // enqueue time 
  //dout(0)<<"###1 enqueue reqid="<<op->get_reqid()<<", rmw_flags="<<op->rmw_flags<<dendl;
  op_shardedwq.queue(
      OpQueueItem(
...
{
  const Message *m = op->get_req();
  //dout(0)<<"###2 dequeue reqid="<<op->get_reqid()<<", rmw_flags="<<op->rmw_flags<<dendl;
  FUNCTRACE(cct);
  OID_EVENT_TRACE_WITH_MSG(m, "DEQUEUE_OP_BEGIN", false);
  if(op->get_dequeued_time() == utime_t{0,0}) { // make sure it's dequeued for first time
    ...
    if (!slot->waiting.empty())
    {
             ...
             slot->waiting.front().get_map_epoch() <= new_osdmap->get_epoch())
      {
        ...
        dout(20) << __func__ << "  " << pgid
                 ...
                 << ", "
                 << (qi.get_map_epoch() < new_osdmap->get_epoch() ? "stale" : "misdirected")
                 << ", dropping" << dendl;
...
void OSD::ShardedOpWQ::_process(uint32_t thread_index, heartbeat_handle_d *hb)
{
  ...
  OpQueueItem item = sdata->pqueue->dequeue(); // pop

  // get the total cost for the dequeued item
  //TODO: print cost for item, and cost for all ops(from MOSDOp)
...
retry_pg:
  ...
  // take next item
  auto qi = std::move(slot->to_process.front()); // deque<OpQueueItem> to_process
  ...
  while (!pg)
  {
    ...
    else if (qi.is_peering())
    {
      if (!qi.peering_requires_pg())
      {
        ...
        qi.run(osd, sdata, pg, tp_handle); // PGOpItem::run, calling osd->dequeue_op(pg, op, handle) 
      }
