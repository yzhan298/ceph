...
ReplicatedBackend::ReplicatedBackend(
    ...
    ObjectStore *store,
    CephContext *cct) : PGBackend(cct, pg, store, coll, c) {}
    ...
    OpRequestRef orig_op)
{
  ...
  // where ceph_tid_t is transcation id, and InProgressOp is client io
  // InProgressOp contains OpRequestRef: 1 transacation to 1 op mapping
  ...
  //dout(0)<<"###3 reqid="<<orig_op->get_reqid()<<dendl;
  // not all op will go through this place
  //get_parent()->get_logger()->tinc(l_osd_op_in_osd_lat, op_in_osd_lat);
  parent->queue_transactions(tls, op.op); // op.op is OpRequestRef
