...
single_dump() {
	...
	clt_bw=$(echo "$(jq '.jobs[0].write.bw' dump-fio-bench-${qdepth}.json) / 1024" | bc -l) # MB/s
	clt_lat=$(echo "$(jq '.jobs[0].write.lat_ns.mean' dump-fio-bench-${qdepth}.json) / 1000000000" | bc -l) # seconds
        ...
        printf '%s\n' $bs $fioruntime $qd $clt_bw $clt_lat $bluestore_kv_sync_lat $bluestore_kvq_lat | paste -sd ',' >> ${DATA_FILE} 
...
printf '%s\n' "bs" "runtime" "qdepth" "bw_mbs" "lat_s" "bluestore_kv_sync_lat" "bluestore_kvq_lat" |  paste -sd ',' > ${DATA_FILE} 
#"kv_sync_p99_lat" "kv_sync_p95_lat" "kv_sync_median_lat" "kv_sync_min_lat" "kvq_p99_lat" "kvq_p95_lat" "kvq_median_lat" "kvq_min_lat" |  paste -sd ',' > ${DATA_FILE}
...
for qd in 48; do
	...
	sudo bin/rbd create --size=30G mybench/image1
	sleep 10 # warmup
	...
	#mv ./aio_lat_vec.csv ./dump_aio_lat_vec-${qd}.csv
	mv ./total_bluestore_simple_write_lat_vec.csv ./dump_total_bluestore_simple_write_lat_vec-${qd}.csv
