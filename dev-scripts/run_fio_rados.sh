...
sudo fio --ioengine=rados --clientname=admin --pool=rados --rw=write --bs=${bs} --runtime=${time} --name=fio_test --iodepth=${qdepth} --size=${size} --write_iolog=fio_iolog --time_based=1
