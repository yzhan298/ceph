...
printf '%s\n' "bs" "runtime" "concurrency" $shard_name "kvq_size" "avg_kvq_size" |  paste -sd ',' >> ${DATA_OUT_FILE} 
...
do_dump() {
	...
	printf '%s\n' $bs $totaltime $qdepth $opq_size $kvq_size $avg_kvq_size | paste -sd ',' >> ${DATA_OUT_FILE}
done
}
...
single_dump() {
	...
	printf '%s\n' $bs $totaltime $qdepth $avg_kvq_size $bluestore_kv_sync_lat $bluestore_service_lat $bluestore_kvq_lat $bluestore_commit_lat | paste -sd ',' >> ${DATA_TOTAL_FILE}
done
}
