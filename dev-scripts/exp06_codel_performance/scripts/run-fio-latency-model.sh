...
DATA_FILE=dump-latency-model.csv  # output file name
pool="mybench"
...
single_dump() {
	...
	bw=$(grep BW dump-fio-bench-${blocksize} |  awk '{print $3}') # | grep -o '[0-9]*')
        ...
        printf '%s\n' $bs $iototal $qd $bluestore_kv_sync_lat $bluestore_kvq_lat $kv_sync_p99_lat $kv_sync_p95_lat $kv_sync_median_lat $kv_sync_min_lat | paste -sd ',' >> ${DATA_FILE}
    done
    #sudo bin/ceph daemon osd.0 perf reset osd
}

#for j in 1 2 3 4 5; do
printf '%s\n' "bs" "iototal" "qdepth" "bluestore_kv_sync_lat" "bluestore_kvq_lat" "kv_sync_p99_lat" "kv_sync_p95_lat" "kv_sync_median_lat" "kv_sync_min_lat" |  paste -sd ',' > ${DATA_FILE}
for i in {0..11}; do
#for i in {11..0}; do
#for qd in 32; do
        #./run.sh $qd $bt
