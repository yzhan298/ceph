...
do_dump() {
	...
	../build/bin/ceph daemon osd.${o} perf dump osd 2>/dev/null >$rawperf
	../build/bin/ceph daemon osd.${o} perf reset osd >/dev/null 2>/dev/null
	...
	../build/bin/ceph daemon osd.${o} perf dump bluestore commit_lat 2>/dev/null >$rawperf
	../build/bin/ceph daemon osd.${o} perf reset bluestore >/dev/null 2>/dev/null
...
../src/stop.sh
../bench_test/my-vstart
# ./my-new-vstart

sleep 10

# delete pool
../build/bin/ceph osd pool delete $pool $pool --yes-i-really-really-mean-it

# create pool named data
../build/bin/ceph osd pool create $pool 100 100
...
for p in $(seq $parallel) ; do
    ../build/bin/rados -b $block_size -o $obj_size -p $pool bench $run_time write -t $concurrent --no-cleanup --run-name ${run_name}-${p} >${out_summary}-${p} &
done
